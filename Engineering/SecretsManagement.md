...
Teams working on Enhanced Security Projects should follow the specific guidance and requirements provided [here](./EnhancedSecurityProjects.md).

The techniques outlined below provide 'good' security and a common pattern for a wide range of languages. They rely on the fact that Azure keeps application settings (the environment) encrypted until just before your app is run. Encryption keys are regularly rotated. They do *not* prevent secrets from existing in plaintext in memory at runtime. Particularly in garbage collected languages those values my exist for longer than the lifetime of the variable.
